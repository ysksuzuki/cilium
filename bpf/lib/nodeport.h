...
static __always_inline int dsr_encap_geneve_opt(struct __ctx_buff *ctx,
						struct iphdr *ip4, __u32 *ifindex)
{
...
int tail_nodeport_ipv4_dsr(struct __ctx_buff *ctx)
{
	...
	ret = dsr_set_opt4(ctx, ip4,
			   ...
			   port, &ohead);
#elif DSR_ENCAP_MODE == DSR_ENCAP_GENEVE
	ret = dsr_encap_geneve_opt(ctx, ip4, &fib_params.l.ifindex);
	if (!IS_ERR(ret)){
		verdict = ret;
		goto out_send;
	}
