...
static __always_inline int
__encap_with_nodeid_opt6(struct __ctx_buff *ctx, __u32 tunnel_endpoint,
			 __u32 seclabel, __u32 dstid, __u32 vni,
			 ...
			 __u32 *ifindex)
{
	int ret_opt;
	int ret = __encap_with_nodeid(ctx, tunnel_endpoint, seclabel, dstid,
				      vni, ct_reason, monitor,
				      ifindex);
	if (unlikely(ret < 0))
		return ret;

	ret_opt = ctx_set_encap_opt6(ctx, gopt);
	if (unlikely(ret_opt < 0))
		return ret_opt;
...
static __always_inline int
__encap_with_nodeid_opt4(struct __ctx_buff *ctx, __u32 tunnel_endpoint,
			 __u32 seclabel, __u32 dstid, __u32 vni,
			 ...
			 __u32 *ifindex)
{
	...
	ret_opt = ctx_set_encap_opt4(ctx, gopt);
	if (unlikely(ret_opt < 0))
		return ret_opt;
...
static __always_inline int
encap_and_redirect_with_nodeid_opt4(struct __ctx_buff *ctx, __u32 tunnel_endpoint,
				    __u32 seclabel, __u32 dstid, __u32 vni,
				    struct geneve_opt4 *gopt,
				    const struct trace_ctx *trace)
{
	...
	int ret = __encap_with_nodeid_opt4(ctx, tunnel_endpoint, seclabel, dstid,
				       vni, gopt, trace->reason, trace->monitor,
				       &ifindex);
	if (ret != CTX_ACT_REDIRECT)
		return ret;
