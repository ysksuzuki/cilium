...
int nodeport_dsr_backend_pktgen(struct __ctx_buff *ctx)
{
	...
	ethhdr__set_macs(l2, client_mac, node_mac);
	...
	*opt = bpf_htonl(DSR_IPV4_OPT_32 | FRONTEND_PORT);
	opt++;
	*opt = bpf_htonl(FRONTEND_IP);
...
int nodeport_dsr_backend_check(const struct __ctx_buff *ctx)
{
	...
	if (*opt != __bpf_ntohl(DSR_IPV4_OPT_32 | FRONTEND_PORT))
		test_fatal("word0 in DSR IP option has changed")
	opt++;
	if (*opt != __bpf_ntohl(FRONTEND_IP))
		test_fatal("word1 in DSR IP option has changed")
...
int build_reply(struct __ctx_buff *ctx)
{
	...
	ethhdr__set_macs(l2, node_mac, client_mac);
 ...
 * DSR backend, and sends it back to the client.
 */
